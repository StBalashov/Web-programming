...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String fileUri = URLDecoder.decode(request.getUri(), "UTF-8");
        if (!(fileUri.endsWith(".png")) && !(fileUri.endsWith(".html")) && !(fileUri.endsWith(".htm"))) {
            fileUri += "/index.html";
        }
        File file = new File(root, fileUri);
        if (file.isFile()) {
