...
//        ArrayList<Method> methods = new ArrayList<>();
        for (Class<?> c = clazz; method == null && c != null; c = c.getSuperclass()) {
            try {
                methodBefore = c.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
                methodAfter = c.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
                ArrayList<Method> methods = new ArrayList<Method>(Arrays.asList(c.getDeclaredMethods()));
                System.out.println(route.getAction());
                methods = methods.stream().filter(method1 -> (method1.getName().equals(route.getAction()))).collect(Collectors.toCollection(ArrayList::new));
        ...
        try {
            ...
            for (int i = 0; i < parameters.length; i++){
                if (parameters[i] == HttpServletRequest.class){
                    arguments[i] = request;
                }
                if (parameters[i] == Map.class){
                    arguments[i] = view;
                }
            }
            method.invoke(page, Arrays.copyOfRange(arguments,0,parameters.length));
